...
Décidé le 02/09/2026 : le design, l'ergonomie et l'UI/UX de ce site reviennent
à Codex ; le reste (serveur, pipeline lexical, logique métier) reste à Claude.

**Jamais les deux en même temps sur le dépôt** - même principe que « ne jamais
mener de front une modification du SSO et une modification visuelle » plus
bas, aggravé ici par le fait que git ne distingue pas les auteurs (tous les
commits de cette machine sont signés « Hugues », voir plus bas « Façon de
travailler attendue ») : si les deux travaillent au même moment, rien ne dit
qui a écrasé quoi. Un seul des deux à la fois, et `git log --oneline -5` en
entrant dans le dépôt reste la première chose à faire.

**La frontière design/logique n'est pas un découpage par dossier.**
`ToolLayout` (le bandeau) est à la fois une contrainte pixel et un composant
qui lit la session ; `QuestionDictee.tsx` et `QuizTool.tsx` mélangent l'écran
et la logique de l'essai/l'aide/le score dans les mêmes fonctions. Sur ces
fichiers-là en particulier, vérifier qui a touché en dernier avant d'écrire.
...
## Ce qu'il faut savoir avant de toucher au design

Consolidé le 02/09/2026, au moment où le design a été confié à Codex : ces
points ne vivaient jusque-là que dans la mémoire de session de Claude, nulle
part dans ce dépôt. À lire avant toute modification visuelle, par l'un ou
l'autre.

- **Code couleur de l'accessibilité dys : LireCouleur (Éduscol), jamais un
  code inventé.** Rouge = voyelle, bleu = consonne, vert = graphème complexe,
  gris = muet. C'est la convention reconnue en France (le BDA déconseille en
  général le vert/rouge isolés, mais LireCouleur fait référence). Le mode
  confort de lecture (activé par l'enseignante, élève par élève) l'associe à
  la police Lexend et à de l'espacement, et masque les mascottes de
  catégorie - le picto ARASAAC du mot, lui, reste affiché : c'est une aide au
  sens, pas une décoration.
- **`src/lib/grammaire.ts` est la seule source des libellés, couleurs et
  mascottes de catégorie grammaticale.** Deux nuances à ne pas perdre en le
  touchant : la mascotte du verbe est `verbe-infinitif` sur la fiche mot et
  le bandeau imprimable, `verbe` générique ailleurs (option `infinitif`) ; la
  fiche mot affiche la nature précise EN PLUS de « Mot invariable », les vues
  compactes la REMPLACENT (`affichageCategorie()`).
- **ARASAAC est sous licence CC BY-NC-SA (non commerciale).** Il fournit tous
  les pictogrammes du site (environ 3300, résolus au build, jamais d'appel
  API depuis le navigateur). Toute piste de commercialisation du logiciel
  lui-même est bloquée tant que cette source - et Manulex, même licence,
  pour le choix des mots - n'est pas remplacée ou renégociée. Vu et noté le
  31/08/2026, rien de tranché depuis.
- **Chantier ouvert, jamais commencé : l'accessibilité RGAA de la modale.**
  `PhonemeInfoModal.tsx` (fiche détail d'un son, utilisée par le clavier et
  le quiz) est l'UNIQUE modale de toute l'app - pas de wrapper `Modal`
  générique, donc la corriger règle tout le monde d'un coup. Il manque les
  critères P0 : `role`/`aria-modal`, titre relié par `aria-labelledby`,
  gestion du focus (pas de piège, retour au clic d'origine à la fermeture),
  fermeture par Échap. Petit chantier, isolé - à faire avant toute
  commercialisation ou dossier Édu-Up, qui exigerait la conformité.
- **Une décision UX a déjà fait l'aller-retour : ne pas la refaire sans
  Hugues.** Cliquer un mot suggéré dans l'aide de la dictée REMPLISSAIT le
  champ de saisie automatiquement, une première fois. Hugues est revenu
  dessus : l'élève doit retaper le mot lui-même, sinon l'aide à la
  mémorisation disparaît. Les suggestions sont maintenant de simples
  `<span>`, non cliquables.
- **Patron d'impression déjà établi**, réutilisé par `FichesMultiples.tsx`
  et `BilanEleveImprimable.tsx` : `window.print()` plus une classe
  `.no-print` sur ce qui ne doit pas sortir sur papier. À reprendre plutôt
  qu'à réinventer.
- **Question ouverte, jamais tranchée** : les icônes emoji du menu
  parlent-elles assez aux CE1 de Camille ? Aucun retour du terrain encore
  là-dessus.
- **Une refonte visuelle peut arriver directement en ligne sans passer par
  ce dépôt.** Déjà arrivé une fois (26/08/2026) : Hugues avait modifié
  l'accueil via un autre outil et transféré directement, sans jamais
  committer - le dépôt local était resté en retard sur ce qui tournait
  vraiment en production. Avant de modifier un fichier visuel, comparer au
  site en ligne (empreinte des fichiers servis, textes affichés) plutôt que
  de supposer que le dépôt est à jour.
